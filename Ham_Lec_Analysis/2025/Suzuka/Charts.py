...
file_path = 'Data_ham.csv'
data_ham = load_csv_with_csv(file_path)

file_path = 'Data_lec.csv'
...
speed_lec = np.array(data_lec[0], dtype=float)
time_lec = np.array(data_lec[1], dtype=str)
distance_ham = np.array(data_ham[2], dtype=float)
speed_ham = np.array(data_ham[0], dtype=float)
time_ham = np.array(data_ham[1], dtype=str)
#if distance_ham[0]<distance_lec[0]:
#    index = bisect.bisect_left(distance_ham, distance_lec[0])
#    delta = time_ham[index:] - time_lec
#if distance_ham[0]>distance_lec[0]:
#    index = bisect.bisect_left(distance_lec, distance_ham[0])
#    delta = time_ham - time_lec[index:]


laptime_ham=pd.to_timedelta(str(time_ham[-1]))
minutes = laptime_ham.components.minutes
seconds = laptime_ham.components.seconds
milliseconds = laptime_ham.components.milliseconds
formatted_ham = f"{minutes:02}:{seconds:02}:{milliseconds:03}"

laptime_lec=pd.to_timedelta(str(time_lec[-1]))
minutes = laptime_lec.components.minutes
seconds = laptime_lec.components.seconds
milliseconds = laptime_lec.components.milliseconds
formatted_lec = f"{minutes:02}:{seconds:02}:{milliseconds:03}"

print(laptime_ham.total_seconds())
print(laptime_lec.total_seconds())


laptime_lec_new=[0]*(len(time_lec))
laptime_ham_new=[0]*(len(time_ham))

for i in range(len(time_lec)):
    laptime_lec_new[i]=pd.to_timedelta(str(time_lec[i])).total_seconds()
for i in range(len(time_ham)):
    laptime_ham_new[i]=pd.to_timedelta(str(time_ham[i])).total_seconds()


if len(distance_lec)<len(distance_ham):
    x_int=distance_lec
    y_int=laptime_lec_new
    delta=[0]*len(laptime_lec_new)
    x_int2=distance_ham
    y_int2=laptime_ham_new
    c=-1

if len(distance_lec)>=len(distance_ham):
    x_int=distance_ham
    y_int=laptime_ham_new
    delta=[0]*len(laptime_ham_new)
    x_int2=distance_lec
    y_int2=laptime_lec_new
    c=1
...
draw.text((30, 110), "Lap Time: " + formatted_lec, font=font_label, fill="white")
draw.text((750, 110), "Lap Time: " + formatted_ham, font=font_label, fill="white")
...
sectors = {
    "Sector 1": (0, circuit_corner_length[7]-100),
    "Sector 2": (circuit_corner_length[7]-100, (circuit_corner_length[13]+circuit_corner_length[14])/2),
    "Sector 3": ((circuit_corner_length[13]+circuit_corner_length[14])/2, max(distance_lec)),
}
...
ax.plot(distance_lec, speed_lec, color='deepskyblue', label='Leclerc', linewidth=2.3, alpha=0.95)
ax.plot(distance_ham, speed_ham, color='crimson', label='Hamilton', linewidth=2.3, alpha=0.95)
...
# Delta trace line
ax2.plot(x_int, delta, color='orange', label='Δ Lap Time (Hamilton – Leclerc)', linewidth=1.8, antialiased=True)

# Fill where Hamilton is ahead
ax2.fill_between(x_int, delta, 0, where=(delta < 0), interpolate=True, color='crimson', alpha=0.25, label='Hamilton Ahead')

# Fill where Leclerc is ahead
ax2.fill_between(x_int, delta, 0, where=(delta > 0), interpolate=True, color='royalblue', alpha=0.25, label='Leclerc Ahead')
...
for x in circuit_corner_length:
    ...
    if c==10 or c==15 or c==16 or c==17:
        ax2.text(x-10, ax2.get_ylim()[1]*0.35, "T"+circuit_corner_number[c],
            color='white', fontsize=10, rotation=90,
            ha='center', va='bottom', fontweight='medium')
        c=c+1
        continue
    ax2.text(x-10, ax2.get_ylim()[1]*0.75, "T"+circuit_corner_number[c],
            color='white', fontsize=10, rotation=90,
            ha='center', va='bottom', fontweight='medium')
    c=c+1

# Y-axis tick labels: add +/− ms
ax2.set_yticks([0, round_sig(max(delta), 3)])
ax2.set_yticklabels(['0', str(round_sig(max(delta), 3))], color='white', fontsize=11)

# Grid
ax2.grid(True, which='major', axis='y', linestyle='--', linewidth=0.3, alpha=0.3)

# Axis and title styling
ax2.set_title("Time Gap (s) (Hamilton – Leclerc)", fontsize=18, color='white', pad=20, weight='bold')
