...
draw.text((30, 70), "Driver 1: CHARLES piaLERC", font=font_label, fill="crimson")
draw.text((750, 70), "Driver 2: LEWIS norILTON", font=font_label, fill="gold")
...
sectors = {
    ...
    "Sector 3": (circuit_corner_length[11]+100, max(distance_pia)),
}
...
# Fill where norilton is ahead
ax2.fill_between(x_int, delta, 0, where=(delta < 0), interpolate=True, color='#FF8700', alpha=0.3, label='Norris Ahead')

# Fill where pialerc is ahead
ax2.fill_between(x_int, delta, 0, where=(delta > 0), interpolate=True, color='#00F076', alpha=0.3, label='Piastri Ahead')
...
for x in circuit_corner_length:
    ...
    if c==12:
        ax2.text(x-10, ax2.get_ylim()[1]*0.50, "T"+circuit_corner_number[c],
            color='white', fontsize=10, rotation=90,
            ha='center', va='bottom', fontweight='medium')
        c=c+1
        continue

    ax2.text(x-10, ax2.get_ylim()[1]*0.73, "T"+circuit_corner_number[c],
            color='white', fontsize=10, rotation=90,
            ha='center', va='bottom', fontweight='medium')
    c=c+1
